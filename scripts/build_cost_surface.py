...
SLOPE_STEEP_THRESHOLD_PERCENT = 30.0
...
def main() -> None:
    ...
    cost = np.where(
        slope_percent > SLOPE_STEEP_THRESHOLD_PERCENT,
        params["terrain_cost_steep"],
        params["terrain_cost_flat"],
    ).astype(np.float32)
    ...
    high_slope_cells = int(np.sum(valid_slope > SLOPE_STEEP_THRESHOLD_PERCENT))
    low_slope_cells = int(np.sum(valid_slope <= SLOPE_STEEP_THRESHOLD_PERCENT))
