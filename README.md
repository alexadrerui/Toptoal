...
## 24) Resolvendo conflitos de merge (PR)

Se o GitHub indicar conflitos neste PR (especialmente em `README.md` e scripts de pipeline),
resolva localmente e suba um novo commit:

```bash
git fetch origin
git checkout <sua-branch>
git merge origin/<branch-base>
# resolver conflitos nos arquivos, validar execução e então:
git add README.md scripts/build_cost_surface.py scripts/build_grid_model.py scripts/download_osm_dem.py
git commit -m "Resolve conflitos de merge com branch base"
git push
```

Recomendação: após resolver, rode ao menos `python -m unittest discover` para garantir que o
merge não quebrou os fluxos mínimos.
